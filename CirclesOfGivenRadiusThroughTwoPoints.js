const hDist = (p1, p2) => 
    Math.hypot(...p1.map((e, i) => e - p2[i])) / 2;
...
switch (Math.sign(s - halfDist)) {
    ...
    case 1:
        if (!halfDist) {
            msg = 'Concident point. Infinite solutions';
        } else {
            const theta = pAng(p1, p2);
            const theta2 = Math.acos(halfDist / s);
            [1, -1].map((e) => solve(theta + e * theta2)).forEach((e) => msg.push(e));
        }
        break;
    case -1:
        msg = 'No intersection. Points further apart than circle diametr';
        break;
    default:
